...
use crate::{CommandRunner, OSASCRIPT};
...
/// （布局见 apps/verge-gpui/scripts/build-macos-app.sh）。
pub fn discover_bundled_helper(resources: &Path) -> Result<HelperBundle, AppError> {
    ...
    if !binary.is_file() {
        return Err(AppError::new(
            ...
            format!(
                "bundled helper not found at {}; package the app with apps/verge-gpui/scripts/build-macos-app.sh first",
                binary.display()
            ),
        ));
    }
