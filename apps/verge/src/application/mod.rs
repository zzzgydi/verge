...
mod command_bus_tests {
    use super::*;
    use crate::domain::{CommandApproval, CommandContext};
...
mod tests {
    ...
    use crate::config::{Profile, ProfileSource, UpdatePolicy};
        ...
        handler
            .execute(
                AppCommand::ImportProfile {
                    ...
                    name: "Local".into(),
                    yaml: "mode: rule\n".into(),
                    source: crate::domain::ProfileSource::Local,
                    update_policy: crate::domain::UpdatePolicy::Manual,
                },
                100,
            )
            .unwrap();
    ...
    fn realtime_logs_redact_secrets_subscriptions_headers_and_home_paths() {
        let event = RealtimeEvent::Log(crate::domain::LogEvent {
            level: "info".into(),
    ...
    fn unhealthy_merge_candidate_restores_previous_merge_and_runtime() {
        ...
        assert_eq!(error.recovery, RecoveryStatus::Restored);
        assert_eq!(store.merge(), &crate::config::MergeConfig::default());
        assert_eq!(store.yaml(&first).unwrap(), "mode: rule\n");
        assert_eq!(core.applied_yaml.len(), 2);
    ...
    fn runtime() -> FakeRuntime {
        FakeRuntime {
            ...
            events: vec![RealtimeEvent::Traffic(crate::domain::TrafficEvent {
                up: 1,
                down: 2,
            })],
            calls: Vec::new(),
...
mod tun_coordination_tests {
    ...
    use crate::domain::{AppError, ErrorCode, HelperStatus, NetworkSettings, RunMode};
    use crate::platform::{HelperControl, TunConfig};
    ...
    fn enabling_tun_without_helper_installed_gives_readable_guidance() {
        ...
        let error = handler
            .execute(crate::domain::RuntimeCommand::SetNetworkSettings { settings: tun_on() })
            .unwrap_err();
    ...
    fn disabling_tun_updates_mihomo_then_tears_down_helper_devices() {
        ...
        handler
            .execute(crate::domain::RuntimeCommand::SetNetworkSettings {
                settings: NetworkSettings {
                    tun_enabled: false,
                    dns_enabled: true,
                    ipv6_enabled: false,
