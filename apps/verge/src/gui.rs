...
use crate::ipc::{ClientEvent, ConnectError, IpcClient};
use crate::platform::{
    MacNotifier, ProcessRunner, daemon_socket_path, redirect_stderr_to_log, spawn_daemon,
};
use crate::ui::{UiAction, UiResponse};
use crate::view::{
...
#[cfg(test)]
#[path = "dialog_mechanism_tests.rs"]
...
fn quit_request() -> crate::ui::UiRequestEnvelope {
    crate::ui::UiRequestEnvelope {
        ...
        request: crate::ui::UiRequest::Profile(AppCommand::QuitApplication),
    }
}
...
pub fn run() {
    ...
    let data_directory = match crate::daemon::data_directory() {
...
fn recovery_hint(lang: Lang, error: &crate::domain::AppError) -> Option<&'static str> {
...
fn toast_for(lang: Lang, response: &UiResponse) -> Option<Notification> {
    ...
    let fail = |error: &crate::domain::AppError| {
